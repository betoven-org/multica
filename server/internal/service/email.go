...
func resolveFromEmail(smtpHost string) string {
	...
	if smtpHost == "" {
		...
		return "noreply@oduscommerce.com.br"
	}
...
func (s *EmailService) SendVerificationCode(to, code string) error {
	...
	if s.smtpHost != "" {
		return s.sendSMTP(to, "Your Odus verification code", body)
	}
	if s.client == nil {
	...
	params := &resend.SendEmailRequest{
		From:    s.fromEmail,
		To:      []string{to},
		Subject: "Your Odus verification code",
		Html:    body,
	}
...
func buildInvitationParams(from, to, inviterName, workspaceName, inviteURL string) *resend.SendEmailRequest {
	...
	return &resend.SendEmailRequest{
		From:    from,
		To:      []string{to},
		Subject: fmt.Sprintf("%s invited you to %s on Odus", subjectInviter, subjectWorkspace),
		Html: fmt.Sprintf(
			`<div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
				<h2>You're invited to join %s</h2>
				<p><strong>%s</strong> invited you to collaborate in the <strong>%s</strong> workspace on Odus.</p>
