...
func (q *Queries) ListWorkspacesWhereUserIsOwner(ctx context.Context, userID pgtype.UUID) ([]Workspace, error) {
	rows, err := q.db.Query(ctx, listWorkspacesWhereUserIsOwner, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
