...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb redis.UniversalClient, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	platformAdmins := splitAndTrim(os.Getenv("PLATFORM_ADMIN_EMAILS"))
	middleware.PlatformAdminEmails = platformAdmins

	signupConfig := handler.Config{
		AllowSignup:              os.Getenv("ALLOW_SIGNUP") != "false",
		AllowedEmails:            splitAndTrim(os.Getenv("ALLOWED_EMAILS")),
		AllowedEmailDomains:      splitAndTrim(os.Getenv("ALLOWED_EMAIL_DOMAINS")),
		PlatformAdminEmails:      platformAdmins,
		...
		r.Route("/api/workspaces", func(r chi.Router) {
			...
			r.Route("/{id}", func(r chi.Router) {
				...
				r.Group(func(r chi.Router) {
					...
					r.Post("/clone-global-items", h.CloneGlobalItems)
